...
typedWords.addEventListener('input', ()=>{
    const arryInput = words.querySelectorAll('span');
    const arryValue = typedWords.value.split('');
    arryInput.forEach((characterSpan, index) => {
...
const startTest = () => {
    ...
    const quote = setOfWords[randomNumber];
    words.innerText = '';
    let date = new Date;
